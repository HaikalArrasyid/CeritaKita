...
const INVERSE_CATEGORY_MAP: Record<string, string> = {
  'WORK': 'Lingkungan Kerja',
  'SCHOOL': 'Pendidikan',
  'HOME': 'Rumah Tangga',
...
export default function EditStoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (user) {
      fetchStory(unwrappedParams.id).then((data) => {
        if (data.authorId !== user.id) {
          router.replace('/');
          return;
        }
        setTitle(data.title);
        setCategory(INVERSE_CATEGORY_MAP[data.category] || CATEGORIES[0]);
    ...
    return (
      <div className="min-h-screen pt-32 pb-20 px-4 flex justify-center items-start">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 text-center max-w-sm w-full">
          ...
          <h2 className="text-xl font-serif font-bold mb-3 text-slate-900">Perlu Akses</h2>
          <p className="text-slate-600 mb-6 text-sm leading-relaxed">
            Anda harus masuk untuk mengedit cerita Anda.
          </p>
          <Button 
            size="md" 
            className="w-full rounded-xl py-6 font-medium text-sm transition-all" 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch(`/stories/${unwrappedParams.id}`, {
        method: 'PATCH',
        body: JSON.stringify({
          title,
          content,
      ...
      router.replace(`/cerita/${unwrappedParams.id}`);
    } catch (err: any) {
  ...
  if (isFetching) {
    return (
      <div className="min-h-[calc(100vh-4rem)] bg-slate-50 py-12 px-4 md:px-6">
        <div className="max-w-3xl mx-auto bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-slate-100 animate-pulse">
          <div className="mb-8 border-b border-slate-100 pb-6">
            <div className="w-1/2 h-8 bg-slate-200 rounded mb-3"></div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[1, 2, 3, 4, 5].map(i => (
                  <div key={i} className="w-24 h-10 bg-slate-200 rounded-full"></div>
                ))}
              </div>
            </div>
            <div>
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-slate-50 py-12 px-4 md:px-6">
      <div className="max-w-3xl mx-auto bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-slate-100">
        <div className="mb-8 border-b border-slate-100 pb-6">
          <h1 className="text-3xl font-serif font-bold text-slate-900 mb-2">Edit Cerita Anda</h1>
          <p className="text-slate-500">Perbarui cerita Anda.</p>
        </div>
